...
(function (global, factory) {
  ...
  var sankey = function () {
    ...
    function scaleSankeySize(graph) {
      ...
      let newHeight =
        currentHeight + totalTopLinksWidth + totalBottomLinksWidth;
          ...
          nodes.forEach(function (node) {
            ...
            if (node.sourceLinks.length || node.targetLinks.length) {
              if (depth == 0 && n == 1) {
                ...
                node.partOfCycle &&
                numberOfNonSelfLinkingCycles(node) > 0
              ) {
    ...
    graph.links.forEach(function (link) {
      ...
      if (link.circular) {
        ...
        if (updateNode) {
          graph.nodes.forEach(function (node) {
            if (
              node.name == link.source.name ||
              node.name == link.target.name
            ) {
              node.circularLinkType = link.circularLinkType;
            }
          });
  ...
  function numberOfNonSelfLinkingCycles(node) {
    ...
    node.sourceLinks.forEach(function (l) {
      sourceCount =
        l.circular && !selfLinking(l) ? sourceCount + 1 : sourceCount;
    });

    let targetCount = 0;
    node.targetLinks.forEach(function (l) {
      targetCount =
        l.circular && !selfLinking(l) ? targetCount + 1 : targetCount;
    });
